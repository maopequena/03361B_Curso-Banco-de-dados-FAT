...
-- DICAS: COUNT(), SUM() e '*', que multiplica valores

DESCRIBE A15_Cliente; -- (id, nome, telefone)
DESCRIBE A15_Fornecedor; -- id, nome, cnpj, telefone, email
DESCRIBE A15_Produto; -- id, id_fornecedor, nome, preco_unitario
DESCRIBE A15_Produto_Vendido; -- id_venda, id_produto, quantidade
DESCRIBE A15_Venda; -- id, id_cliente, data_venda, desconto

-- 1. Nome e preço do produto + Nome, CNPJ e telefone do respectivo fornecedor
SELECT 	p.nome AS Produto,
		p.preco_unitario AS 'Preço unit.',
...
-- 2. Nome e telefone do cliente + Quantas compras cada cliente fez
SELECT c.nome AS Cliente, c.telefone AS Telefone, COUNT(DISTINCT v.id) AS 'Qtdd de compras'
	FROM A15_Cliente AS c
INNER JOIN A15_Venda AS v
	ON c.id = v.id_cliente
GROUP BY c.nome, c.telefone;

-- 3. DESAFIO: Id e data de cada venda + Nome e telefone do cliente + Valor total da venda
SELECT 	v.id AS 'ID Venda',
		v.data_venda AS 'Data da venda',
        c.nome AS Cliente,
        c.telefone AS Telefone,
        ROUND(SUM(pv.quantidade * p.preco_unitario * (1 - v.desconto)), 2) AS 'Total'
FROM A15_Venda AS v
INNER JOIN A15_Cliente AS c
	ON c.id = v.id_cliente
INNER JOIN A15_Produto_Vendido AS pv
	ON pv.id_venda = v.id
INNER JOIN A15_Produto AS p
	ON p.id = pv.id_produto
GROUP BY v.id-- , v.data_venda, c.nome, c.telefone
ORDER BY c.nome ASC;